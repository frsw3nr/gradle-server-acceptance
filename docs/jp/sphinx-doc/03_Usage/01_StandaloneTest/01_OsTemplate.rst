...
* 検査用 PC の設定準備
   ...
   - :doc:`../../02_Installation/01_TestPCSetup` の手順で事前に検査用PCのセットアップをしてください
...
設定した検査ルールに従って採取した値のチェックをします。以下のチェックを想定しています。
...
検査ルールがOKのときはセルが「グリーン」に配色され、検査ルールがNGのときはセルが「ピンク」に配色されます。
...
「サーバチェックシート.xlsx」を開き、シート「検査ルール」の入力列に
比較対象の検査対象サーバの代表のサーバ名を追加します。

   * compare_server

     結果を比較する対象サーバ。
     事前に「getconfig -u local」でデータベースに検査結果を登録したサーバ名を入力してください。
     まだ、データベース登録をしておらず、複数の類似設定のサーバの比較をする場合は、
     代表サーバのサーバ名を入力して下さい。

   * compare_source

      結果を比較するソース。以下を入力して下さい。

      * 「actual」 : 実行結果から
      * 「local」 : ローカルデータベースから。事前に「getconfig -u local」で検査結果の登録が必要です
...
上記値を入力後、 getconfig コマンドを実行すると、チェックシートの検査結果に比較結果を追加します。
    ...
    * 次の列から、比較結果の列で値が一致しているセルは「水色」に配色されます。
    * 検査ルールが設定されている行は、上記の検査ルールにしたがって「グリーン」、「ピンク」に配色されます。
