...
grant all privileges on redmine2.* to redmine2@"%" identified by 'getperf' with grant option;
flush privileges;
exit
...
cd ~/redmine/plugins/
mv redmine_getconfig ~/work/tmp
ln -s ~/work/redmine_getconfig/redmine/plugins/redmine_getconfig/ redmine_getconfig
cd ..
bundle install
...
redmine にadmin/admin でログイン
管理者パスワードの設定とロケールを日本語に設定

設定すると、日本語向けのマスターデータ登録が実行される
...
#domains -> platforms
alter table domains rename to platforms;

#platforms カラム変更
alter table platforms change domain_name platform_name varchar(24);

#metrics カラム変更
alter table metrics change domain_id platform_id int(11);
