...
class NetAppDataONTAP extends InfraTestSpec {
    ...
    def network_interface(session, test_item) {
        def lines = exec('network_interface') {
            def result = session.execute 'network interface show'
            new File("${local_dir}/network_interface").text = result
            return result
        }
        def result = 'OK'
        def infos = [:].withDefault{[:]}
    ...
    def aggregate_status(session, test_item) {
        def lines = exec('aggregate_status') {
            def result = session.execute 'aggr show -owner-name *'
            new File("${local_dir}/aggregate_status").text = result
            return result
        }
        def result = 'OK'
        def infos = [:].withDefault{[:]}
    ...
    def df(session, test_item) {
        def lines = exec('df') {
            def result = session.execute 'df'
            new File("${local_dir}/df").text = result
            return result
        }
        def result = 'OK'
        def infos = [:].withDefault{[:]}
    ...
    def version(session, test_item) {
        def lines = exec('version') {
            def result = session.execute 'version'
            new File("${local_dir}/version").text = result
            return result
        }
        def result = 'OK'
        def infos = [:].withDefault{[:]}
    ...
    def vserver(session, test_item) {
        def lines = exec('vserver') {
            def result = session.execute 'vserver show'
            new File("${local_dir}/vserver").text = result
            return result
        }
        def result = 'OK'
        def infos = [:].withDefault{[:]}
